    ...
    <>
      ...
      <nav
        className={`fixed top-[3px] left-0 right-0 z-[200] transition-all duration-300 ${
          scrolled
            ? 'bg-white shadow-[0_2px_24px_rgba(0,0,0,0.08)] py-2'
            : 'bg-white/[0.97] backdrop-blur-xl border-b border-brand-pink/10 py-3'
        }`}
      ...
      >
          ...
          <ul className="hidden lg:flex items-center gap-1 list-none m-0 p-0">
            {navLinks.map(([href, label]) => (
              <li key={href}>
                <Link
                  href={href}
                  className="relative font-outfit text-[0.7rem] tracking-[0.14em] uppercase text-brand-black/55 no-underline font-normal px-4 py-2 transition-colors duration-300 hover:text-brand-pink group block"
                >
                  {label}
        ...
        >
          <ul className="list-none m-0 p-0 px-4 sm:px-8 py-4 flex flex-col gap-1 bg-white">
            {navLinks.map(([href, label]) => (
              <li key={href}>
                <Link
                  href={href}
                  onClick={() => setMenuOpen(false)}
                  className="block font-outfit text-[0.72rem] tracking-[0.14em] uppercase text-brand-black/60 no-underline py-3 border-b border-brand-pink/8 transition-colors duration-200 hover:text-brand-pink font-normal"
                >
                  {label}
                </Link>
              </li>
